...
const InsideRoom = () => {
    ...
    const [isIamSubmit, setIsIamSubmit] = useState(false)
    ...
    const kickedOut = (opponentId, opponentNanoId) => {
        if (!socketConnection || !location.state) return

        socketConnection.once("kicked_Err", (kickedData) => {
            toast.error(kickedData?.message)
        })

        socketConnection.emit("kicked-out", {
            roomId: location.state?.roomId,
            opponentId: opponentId,
            opponentNanoId: opponentNanoId
        })
    }
    ...
    const stopCount = () => {
        ...
        socketConnection.emit("stop-count", {
            roomId: location.state?.roomId
        })
    }
    ...
    useEffect(() => {
        ...
        socketConnection.once("kicked-success", (kickedData) => {

            toast.success(kickedData?.message)

            if (kickedData?.kicked) {
                localStorage.setItem("left", "done")
                navigate("/battle-1v1")
            }
            else {
                setData((prev) => {
                    if (!prev) return null
                    const x = prev?.player.filter((c) => c?.userId !== kickedData?.opponentId)
                    return {
                        ...prev,
                        player: x
                    }
        ...
        socketConnection.on("stop-success", (stopData) => {
            toast.success(stopData?.message)
            setCount(null)
        })
        ...
        return () => {
            ...
            socketConnection.off("kicked-success")
            socketConnection.off("stop-success")
        }
                    ...
                    <section className="h-[calc(100vh-70px)] w-full flex items-center justify-center bg-gradient-to-br from-[#fdf3f3] to-[#c9d8f0] p-4 overflow-y-hidden">
                        <div className="bg-white/70 backdrop-blur-xl shadow-2xl rounded-2xl mb-[20px] px-10 py-12 max-w-xl w-full text-center border border-white/40 animate-fadeIn">
                            ...
                            <button
                                ...
                                className="px-8 py-3 bg-red-700 text-white text-lg font-semibold rounded-xl hover:bg-red-800 transition-all shadow-lg hover:shadow-xl cursor-pointer"
                            >
                                Go Back
                            </button>
                        </div>
                                        ...
                                        <div>
                                            {isAdminIam ? (
                                                v?.admin ? (
                                                    ...
                                                    <span onClick={() => kickedOut(v?.userId, v?.user_nanoId)} className="px-3 py-1 text-xs font-semibold bg-red-100 text-red-600 rounded-full shadow-sm cursor-pointer hover:bg-red-200">
                                                        Kick
                                                    </span>
                                                )
                                            ) : (
                            ...
                            count && (
                                <section className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm z-50">
                                    <div className="bg-white rounded-2xl shadow-xl px-10 py-8 flex flex-col items-center gap-6 animate-scaleIn">
                                        ...
                                        <button
                                            onClick={() => stopCount()}
                                            className="px-5 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl shadow-md transition-all cursor-pointer"
                                        >
                                            Cancel
                                        </button>
                                    </div>
